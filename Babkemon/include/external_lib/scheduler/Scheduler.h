...
namespace Bosma {
  ...
  class Task {
  public:
    explicit Task(std::function<bool()> &&f, bool recur = false, bool interval = false) :
  ...
  public:
    explicit InTask(std::function<bool()> &&f) : Task(std::move(f)) {}
  ...
  public:
    EveryTask(Clock::duration time, std::function<bool()> &&f, bool interval = false) :
  ...
  public:
    CronTask(const std::string &expression, std::function<bool()> &&f) : Task(std::move(f), true), cron(expression) {}
